...
<body>
	...
	<div class="container-inf">
		...
		<form class="form-inf" action="../admin/updateprd/submit?idmh=${prd.getMaMH() }"
			method="post">
			<h1 class="title-inf">Cập nhật sản phẩm</h1>
			<div class="prop-row">
				<label for="" class="lable-inf">Tên sản phẩm</label> <input
					type="text" name="TenMH" required class="input-inf"
					placeholder="Nhập tên sản phẩm..." value="${prd.getTenMH() }">
			</div>
			...
			<div class="prop-row">
				...
				<textarea type="text" name="MoTa" required="required" class="area-inf"
					placeholder="Mô tả sản phẩm...">${prd.getMoTa() }</textarea>
			</div>
			...
			<div class="prop-row">
				...
				<input type="text" maxlength="15" name="TrangThai" required="required" class="input-inf"
					placeholder="Mô tả sản phẩm..." value="${prd.getTrangThai() }"/>
			</div>
			...
			<div class="prop-row">
				...
				<select name="MaLMH" class="input-inf">
					...
					<c:choose>
				         ...
				         <c:when test="${item.getMaLMH()==prd.getLoaiMH() }">
				            <option selected="selected" value="${item.getMaLMH() }">${item.getTenLMH()}</option>
				         </c:when>
			...
			<div class="prop-row">
				<label for="" class="lable-inf">Đường dẫn hình ảnh</label> <input
					...
					class="input-inf" id="srcimg" placeholder="Chọn đường dẫn hình ảnh - bản xem trước sẽ xuất hiện	..." value="${prd.getHinhAnh() }">
			</div>
